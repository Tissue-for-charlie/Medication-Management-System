...
class SupplierManagementSystem:
    def __init__(self, root):
        self.root = root
        self.root.title("供应商管理系统")
        self.root.geometry("1000x600")
        
        # 配置数据库
        self.db_path = "suppliers.db"
        self.create_db()
        ...
        ttk.Label(self.header_frame, text="供应商管理", font=("微软雅黑", 16, "bold")).pack(side=tk.LEFT)
        ...
        self.delete_button = ttk.Button(self.button_frame, text="删除", command=self.delete_supplier, style="Danger.TButton")
        ...
        ttk.Label(self.search_frame, text="供应商名:").pack(side=tk.LEFT, padx=10)
        self.search_var = tk.StringVar()
        ttk.Entry(self.search_frame, textvariable=self.search_var, width=30).pack(side=tk.LEFT, padx=10)
        ttk.Button(self.search_frame, text="查询", command=self.search_supplier, style="Primary.TButton").pack(side=tk.LEFT, padx=10)
        ...
        # 表格
        columns = ("id", "name", "contact", "status", "created_at", "updated_at", "action")
        self.tree = ttk.Treeview(self.table_frame, columns=columns, show="headings")
        
        # 设置列标题
        self.tree.heading("id", text="序号")
        self.tree.heading("name", text="供应商名")
    ...
    def open_add_window(self, supplier_id=None):
        # 打开新增/编辑供应商窗口
        AddSupplierWindow(self.root, self, supplier_id)
    ...
    def delete_supplier(self):
        ...
        if not selected_item:
            messagebox.showinfo("提示", "请选择要删除的供应商")
            return
        if messagebox.askyesno("确认", "确定要删除选中的供应商吗？"):
    ...
    def refresh_list(self):
        ...
        # 获取总数
        if search_text:
            cursor.execute("SELECT COUNT(*) FROM suppliers WHERE name LIKE ?", (f"%{search_text}%",))
        else:
        ...
        if search_text:
            cursor.execute("SELECT * FROM suppliers WHERE name LIKE ? ORDER BY id DESC LIMIT ? OFFSET ?", 
                         (f"%{search_text}%", self.page_size, offset))
        else:
            cursor.execute("SELECT * FROM suppliers ORDER BY id DESC LIMIT ? OFFSET ?", (self.page_size, offset))
        suppliers = cursor.fetchall()
        
        for i, supplier in enumerate(suppliers, start=offset+1):
            # 创建操作按钮
            action_frame = ttk.Frame()
            edit_button = ttk.Button(action_frame, text="编辑", command=lambda sid=supplier[0]: self.open_add_window(sid), style="Primary.TButton")
            edit_button.pack(side=tk.LEFT, padx=2)
            delete_button = ttk.Button(action_frame, text="删除", command=lambda sid=supplier[0]: self.delete_supplier_by_id(sid), style="Danger.TButton")
...
class AddSupplierWindow:
    def __init__(self, parent, main_app, supplier_id=None):
        self.parent = parent
        self.main_app = main_app
        self.supplier_id = supplier_id
        
        self.window = tk.Toplevel(parent)
        self.window.title("新增" if supplier_id is None else "编辑")
        ...
        # 供应商名
        ttk.Label(self.form_frame, text="*供应商名").grid(row=0, column=0, sticky=tk.W, pady=15)
        self.name_var = tk.StringVar()
        ttk.Entry(self.form_frame, textvariable=self.name_var, width=40).grid(row=0, column=1, pady=15)
        
        # 联系方式
        ttk.Label(self.form_frame, text="*联系方式").grid(row=1, column=0, sticky=tk.W, pady=15)
        self.contact_var = tk.StringVar()
        ttk.Entry(self.form_frame, textvariable=self.contact_var, width=40).grid(row=1, column=1, pady=15)
        
        # 地址
        ttk.Label(self.form_frame, text="*地址").grid(row=2, column=0, sticky=tk.W, pady=15)
        self.address_var = tk.StringVar()
        ttk.Entry(self.form_frame, textvariable=self.address_var, width=40).grid(row=2, column=1, pady=15)
        
        # 备注
        ttk.Label(self.form_frame, text="备注").grid(row=3, column=0, sticky=tk.W, pady=15)
        self.note_var = tk.StringVar()
        ttk.Entry(self.form_frame, textvariable=self.note_var, width=40).grid(row=3, column=1, pady=15)
        ...
        ttk.Button(self.button_frame, text="确认", command=self.save_supplier, style="Accent.TButton").pack(side=tk.RIGHT, padx=10)
    ...
    def load_supplier_data(self):
        ...
        if supplier:
            ...
            self.contact_var.set(supplier[2])
            self.address_var.set(supplier[3])
            self.note_var.set(supplier[4])
    ...
    def save_supplier(self):
        ...
        if self.supplier_id:
            # 更新
            cursor.execute('''
                UPDATE suppliers SET name=?, contact=?, address=?, note=?, updated_at=?
                WHERE id=?
            ''', (name, contact, address, note, now, self.supplier_id))
        else:
            # 新增
            cursor.execute('''
                INSERT INTO suppliers (name, contact, address, note, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (name, contact, address, note, now, now))
